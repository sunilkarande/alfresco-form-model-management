 ...
 * Mike Priest Edit*
 * -----------------
 * Added hasWriteAccess
 * Added hasAssociations
 * Added isGridEnabled
 * Added STUDENTID key param ( replaceRegExpression() )
 * -----------------
 * Search Component
 *
...
/**
 *  @change added
 *  Returns boolean if the search grid is enabled
 */
model.isGridEnabled = false;
function isGridEnabled(){
	...
	if(args.fields){
		...
		var decode = decodeURIComponent(args.fields);
		model.gridFields = decode.split(",");
	}

	//Set to model
	model.isGridEnabled = enabled;
	return enabled;
}

/**
 *  @change added
 *  Add properties set in FM to result set
 */
function setGridProperties(node, item){

	item.fields = model.gridFields.join(",");
	var fields = model.gridFields;
	...
	if(fields.length > 0)
	{
		...
		for(i in fields)
		{
			item.nodeId = node.properties["sys:node-uuid"];

			if(node.properties[fields[i]]){

				if(node.properties[fields[i]] == ""){
					item.properties.push("Not available");

				}else if( node.properties[fields[i] ] instanceof Array){
					item.properties.push( node.properties[fields[i]].join(", ") );
				}else{
					if( fields[i] == "ua:institution" ) {
						var institutionsJson = companyhome.childByNamePath("/Data Dictionary/Models/Constraints/Institutions.json");
						var institutions = eval("(" + institutionsJson.content + ")");

						for (var j = 0; j < institutions.length; j++) {
							if (node.properties[fields[i]] == institutions[j].key) {
								node.properties[fields[i]] = institutions[j].label;
								break;
							}
						}
					}

					item.properties.push( node.properties[fields[i]] );
				}
			}else{
				item.properties.push("Not available");
			}
...
function getRepositoryItem(folderPath, node)
{
       ...
       !(node.qnamePath.match(DISCUSSION_QNAMEPATH+"$") == DISCUSSION_QNAMEPATH))
   {
      if (node.isContainer || node.isDocument)
      {
		 ...
		 //@change add associations and write access
         item.hasWriteAccess = node.hasPermission("Write");
		 item.hasAssociations = node.hasAspect('asm:peer');

		 //@change add search grid
		 var gridEnabled = isGridEnabled();
		 if(gridEnabled) item = setGridProperties(node, item);
...
/**
 * @change add
 * Checks for keyword and returns the correct query param
 * @example STUDENTID:123 -> ua:studentid:"123"
 */
function replaceRegExpression(regex, prop, queryString){

  	var matches = queryString.match(regex);

	if(matches){
		//Get rid of old val
		var queryString = queryString.replace(regex, "");
		var propSplit = matches[0].split(":");
 		var propString = prop + ':"' + propSplit[1] + '"';

 		//Append and if keyword also entered
 		if(queryString.length > 0){
 		 	queryString = propString + " AND " + queryString;
 		}else{
...
function getSearchResults(params)
{
   ...
   if (term !== null && term.length !== 0)
   {
      ...
      term = replaceRegExpression(/(|[\+])PERSONID:(\w*)\b/i, "ua:personid", term);
      term = replaceRegExpression(/(|[\+])STUDENTID:(\w*)\b/i, "ua:personid", term);

      ftsQuery = term + " ";
   }
//@change Alfresco 4.1.4
