...
function convertJsonToXml(modelObj){
	var mString = "";

	mString = '<?xml version="1.0" encoding="UTF-8"?>';
	mString += '<!-- First namespace prefix in namespace array is used for model prefix -->';

	mString += '<model name="'+ modelObj.namespaces[0]['@prefix']+':'+modelObj.name +'" xmlns="http://www.alfresco.org/model/dictionary/1.0">';

	mString += '	<description>'+ Encoder.htmlEncode(modelObj.description,true) +'</description>';
	mString += '	<author>'+ Encoder.htmlEncode(modelObj.author,true) +'</author>';
	mString += '	<version>'+ Encoder.htmlEncode(modelObj.version, true) +'</version>';
	mString += '	<imports>';

	for(x in modelObj.imports){
		mString += '	<import uri="'+ modelObj.imports[x]['@uri'] +'" prefix="'+ modelObj.imports[x]['@prefix'] + '" />';
	}
	...
	for(x in modelObj.namespaces){
		mString += '	<namespace uri="'+ modelObj.namespaces[x]['@uri'] +'" prefix="'+ modelObj.namespaces[x]['@prefix'] + '" />';
	}
	mString += '	</namespaces>';
	...
	if(modelObj.types){
		mString += '	<types>';
		for(x in modelObj.types){

			mString += '	<type name="'+ modelObj.types[x]['name'] +'">';
			mString += '		<title>'+ modelObj.types[x]['title'] +'</title>';
			mString += '		<parent>'+modelObj.types[x]['parent']+'</parent>';
			mString += '	</type>';
		}
		mString += '	</types>';
	}
	...
	if(modelObj.constraints){
		mString += '	<constraints>';
		for(x in modelObj.constraints){

			mString += '<constraint name="'+ modelObj.constraints[x]['name'] +'" type="'+ modelObj.constraints[x]['@type'] + '">';
			mString += '	<parameter name="allowedValues">';
			mString += '		<list>';

			var paramlist = modelObj.constraints[x].parameter.list.value;
			for(a in paramlist){
				mString += '			<value>'+paramlist[a]+'</value>';
			}
			mString += '		</list>';
